...
from recipes.models import Favorite, Ingredient, Recipe, Tag, ShoppingCartRecipe, IngredientRecipe
    ...
    def delete(self, request, *args, **kwargs):
        ...
        try:
            instance = get_object_or_404(
                Subscription, subscriber=self.request.user, subscribed_to=subscribed_to
            )
        except Http404:
            return Response(
...
def download_shopping_cart(request):
    listi = IngredientRecipe.objects.filter(
        recipe__shopping_carts__user=request.user).values('ingredient__name').annotate(num=Sum('amount'))
    print('\n'.join(f' - {name.title()} ({measurement_unit}) -> {num} ' for name, num, measurement_unit in listi.values_list(
